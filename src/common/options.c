...
int parse_options(int argc, char **argv, options_t *opts) {
    ...
    /* whole is gone as a selectable mode: classic mpi/master-slave always use the
     * children-direct partitioned bodyforce; single/tiled keep the whole kernel. */
    opts->force_deps =
        (opts->algorithm == ALGO_CLASSIC &&
         (opts->backend == BACKEND_MPI || opts->backend == BACKEND_MASTER_SLAVE))
        ? FORCE_DEPS_PARTITIONED : FORCE_DEPS_WHOLE;
